...
async fn test_pipeline_runs_with_test_server() {
    ...
    // Verify all objects in array have all required fields
    let array = json.as_array().unwrap();
    for item in array {
        assert!(item.get("issue").is_some(), "missing 'issue' field");
        assert!(item.get("title").is_some(), "missing 'title' field");
        ...
        assert!(item.get("agents").is_some(), "missing 'agents' field");
        assert!(
            item.get("duration_s").is_some(),
            "missing 'duration_s' field"
        );
    }

    // Additional validation: if stub returns data, verify structure of first object
    if array.len() > 0 {
        let first = &array[0];
